...
import numpy as np
from scipy.sparse import spdiags, csr_matrix, dia_matrix
...
class Difference_Matrix():
    ...
    def sparse_banded(self):
        ...
        # confirm this works
        sparse_mat = dia_matrix(
            (self.DDT_diag, range(-self.k, self.k+1)), shape=(self.n-2, self.n-2))
    ...
    def extract_matrix_diagonals(self, n, k):
        ...
        if k == 0:
            ...
            diag = spdiags([i*np.ones(n)
                           for i in coeff], np.arange(0, k+1), n-2, n)
