...
def compute_spline_derivative(spline: Regression_Spline_Estimator, degree: int):

    """Computes the kth derivative of a spline returning a new spline object"""

    assert degree > 0, "order must be greater than 0"

    # recursively compute the kth derivative of a spline
    if degree > 1:
        return compute_spline_derivative(compute_spline_derivative(spline, degree - 1), 1)
    ...
    derivative_basis = basis.B(x, m=spline.degree)
    ...
    # (K+2M-2-3)
    knot_diff = np.array(differences(repeated_knots[1:-1], spline.degree)).reshape(-1, 1)
    ...
    Q[index + 1] = spline.degree * p_diff[index] * (1 / knot_diff[index])

    # compute the new coefficients
    estimate = derivative_basis.dot(Q)

    regression_spline = Regression_Spline_Estimator(x, estimate, knots, degree=spline.degree - 1)
